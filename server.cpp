...
void signalHandler(int signum) {  
    std::cout << "SIGPIPE signal received. Client disconnected." << std::endl;  
    signalReceived = true;  
}  
class TcpServer{
public:
...
void TcpServer::mains() {  
  ...
  while(true){ 
   ...
   write(new_socket,(char*)&fileheader,sizeof(fileheader));
    // 读取文件内容并发送给客户端  
    while ((bytes_received = read(file_fd, buffer, BUFFER_SIZE)) > 0&& !signalReceived) {  
